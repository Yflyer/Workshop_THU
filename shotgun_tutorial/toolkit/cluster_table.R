...
folder = 'cov'
#suffix = '.covs.tsv'
cluster_info = vroom('clu.tsv')
#sample_list = read.table('sample_list.txt',stringsAsFactors = F) %>% unlist()

colnames(cluster_info)=c('rep_seq','mem_seq')
mem_seq = cluster_info %>% distinct(mem_seq) 
rep_seq = cluster_info %>% distinct(rep_seq) 

sample_list = fs::dir_ls(folder,type='file')
...
vroom_write(cluster_table, "cluster_table.tsv")
